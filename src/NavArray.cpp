...
void NavArray::callback(const nav_msgs::Path msg) {
    //Deletes the previous markers in rviz
    for ( auto m = marker_array.markers.begin(); m != marker_array.markers.end(); ++m)
        m->action = visualization_msgs::Marker::DELETE;
    visual_tools->publishMarkers( marker_array );
    ...
    if (size > 1) {
        ...
        for (int i = frequency; i <= size - 1; i = next) {
            ...
            next = (i == final_pose && final_pose != size - 1) ? (size -1) : (i + frequency);
        }
        visual_tools->publishMarkers( marker_array );
    }
}